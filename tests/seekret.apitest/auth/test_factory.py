...
def _patch_methods(monkeypatch):
    monkeypatch.setattr(seekret.apitest.auth.factory, '_SUPPORTED_AUTH_METHODS', {})
...
def test_register_and_call_factory():
    ...
    assert seekret.apitest.auth.create_auth(test_auth_type_name, test_auth_data) is test_auth_method_sentinel
...
def test_register_factory_with_existing_name_causes_runtime_error():
    ...
    pytest.raises(RuntimeError, seekret.apitest.register_auth_method_factory, None, type_name='existing')


def test_create_unsupported_type_causes_value_error():
    pytest.raises(ValueError, seekret.apitest.auth.create_auth, 'does-not-exist', {})
